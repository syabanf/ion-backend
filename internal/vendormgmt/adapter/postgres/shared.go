// Package postgres implements driven adapters for the vendor bounded
// context against PostgreSQL.
//
// Conventions match reseller / partnership / enterprise:
...
//   - Queries scoped to the `vendor.*` schema.
package postgres
...
// errors. Mirrored from reseller/adapter/postgres/shared.go (kept
// separate to keep zero cross-context imports — vendor will move to
// its own service binary).
